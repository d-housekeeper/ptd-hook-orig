...
void initDLOpenHook() {
  ...
  GumInvocationListener *listener = reinterpret_cast<GumInvocationListener *>(
      g_object_new(DLOPEN_TYPE_LISTENER, nullptr));
...
static void dlopen_listener_on_enter(GumInvocationListener *listener,
                                     GumInvocationContext *ic) {
  DLOpenOperation *op = GUM_IC_GET_INVOCATION_DATA(ic, DLOpenOperation);
  const char *loadedLibName =
      (const char *)gum_invocation_context_get_nth_argument(ic, 0);
  op->loadedLibName = new std::string(loadedLibName);
}

static void dlopen_listener_on_leave(GumInvocationListener *listener,
                                     GumInvocationContext *ic) {
  DLOpenOperation *op = GUM_IC_GET_INVOCATION_DATA(ic, DLOpenOperation);
  __android_log_print(ANDROID_LOG_DEBUG, androidLogTag, "dlopen %s",
                      op->loadedLibName->c_str());
